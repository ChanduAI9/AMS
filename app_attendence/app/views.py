...
def user_register(request):
    ...
    if request.method == 'POST':
        ...
        if form_data.is_valid():
            ...
            try:
                profile=UserProfile.objects.get(data=new_user)
            except:
                profile_data= None
            if profile_data is None:
                UserProfile(data=new_user,date_of_birth=data['date_of_birth'],contact=data['contact'],branch_name=data['branch_name'],gender=data['gender'],profile_image=request.FILES['profile_image']).save()
            else:
                UserProfile.objects.filter(id=profile.id).update(data=new_user,date_of_birth=data['date_of_birth'],contact=data['contact'],branch_name=data['branch_name'],profile_image=request.FILES['profile_image'])
                profile_image.save()
            ...
            access['form_data']=form_data
    return render(request,'signuppage.html',access)
            ...
            if request.POST:
                ...
                if user is not None:
                    if user.is_active:
                        ...
                        access['message']="incorrect details"
                else:
                    access['message']="incorrect details"
            else:
                access['message']="user is not valid"
        else:
            access['message']="Invalid user details"
    return render("login_user.html",access)
